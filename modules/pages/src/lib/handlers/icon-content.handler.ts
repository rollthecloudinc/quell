...
export class IconContentHandler implements ContentHandler {
  ...
  toObject(settings: Array<AttributeValue>): Observable<QIcon> {
    return of(new QIcon(this.attributeSerializer.deserializeAsObject(settings)));
  }

  buildSettings(icon: QIcon): Array<AttributeValue> {
    return this.attributeSerializer.serialize(icon, 'root').attributes;
  }
