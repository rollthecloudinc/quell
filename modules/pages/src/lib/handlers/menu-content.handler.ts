...
import { QMenu } from '../models/plugin.models';

@Injectable({
  providedIn: 'root'
})
export class MenuContentHandler implements ContentHandler {
  ...
  toObject(settings: Array<AttributeValue>): Observable<QMenu> {
    return of(new QMenu(this.attributeSerializer.deserializeAsObject(settings)));
  }

  buildSettings(menu: QMenu): Array<AttributeValue> {
    return this.attributeSerializer.serialize(menu, 'root').attributes;
  }
