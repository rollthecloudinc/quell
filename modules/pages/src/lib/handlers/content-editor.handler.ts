...
import { ContentEditorOptions } from '../models/plugin.models';

@Injectable({
  providedIn: 'root'
})
export class ContentEditorHandler implements ContentHandler {
  ...
  toObject(settings: Array<AttributeValue>): Observable<ContentEditorOptions> {
    return of(new ContentEditorOptions(this.attributeSerializer.deserializeAsObject(settings)));
  }

  buildSettings(options: ContentEditorOptions): Array<AttributeValue> {
    return this.attributeSerializer.serialize(options, 'root').attributes;
  }
