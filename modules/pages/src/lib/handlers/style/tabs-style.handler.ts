...
export class TabsStyleHandler implements StyleHandler {
  ...
  alterResolvedPanes(
    ...
    return forkJoin(nestedPages$).pipe(
      map(pages => pages.reduce((p, [cp, c]) => [ ...p, new Pane(c.panels[0].panes[0]), new Pane({ ...cp, settings: this.panelHandler.buildSettings(new PanelPage({ ...c, panels: [ ...c.panels.map((p2, i) => i !== 0 ? p2 : new Panel({ ...p2, panes: p2.panes.map(p3 => new Pane(p3)).filter((_, i2) => i2 !== 0) })) ] })) }) ], [])),
      tap(rebuiltPanes => {
