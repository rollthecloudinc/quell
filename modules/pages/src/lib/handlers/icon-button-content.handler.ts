...
export class IconButtonContentHandler implements ContentHandler {
  ...
  toObject(settings: Array<AttributeValue>): Observable<IconButton> {
    return of(new IconButton(this.attributeSerializer.deserializeAsObject(settings)));
  }

  buildSettings(button: IconButton): Array<AttributeValue> {
    return this.attributeSerializer.serialize(button, 'root').attributes;
  }
